...
*******************************************************************/
{
	...
	twi_tx(0x21);	// Internal osc on (page 10 HT16K33) // 0010 0001 -> System Setup Register (page 10)
	twi_stop();

	twi_start();
	twi_tx(0xE0);	// Display I2C address + R/W bit //1110 0000
	twi_tx(0xA0);	// HT16K33 pins all output //1010 0000 ->ROW/INT set (page 10)
	twi_stop();

	twi_start();
	twi_tx(0xE0);	// Display I2C address + R/W bit //1110 0000
	twi_tx(0b11101111);	// Display Dimming 4/16 duty cycle //1110 0011 -> Digital Dimming Data Input (page 15)
	twi_stop();

	twi_start();
	twi_tx(0xE0);	// Display I2C address + R/W bit //1110 0000
	twi_tx(0x81);	// Display OFF - Blink On //1000 0001 ->Display Setup Register (page 11)
	twi_stop();
...
void setBrightness(int brightness){
	sendData((0b11100000+(brightness)));
}
